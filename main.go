...
func getCoinPrice(coinType string) string {
	...
	startOfTheDay := time.Date(init.Year(), init.Month(), init.Day(), int(0), int(0), int(0), int(0), location)
	now := time.Date(init.Year(), init.Month(), init.Day(), init.Hour(), init.Minute(), init.Second(), init.Nanosecond(), location)
	...
	table := "\n\n*24H:*\t\t\t\t\t\t\t`" + fmt.Sprintf("%v", (math.Round(percentageChange(product24h[0].Open, product24h[0].Close)*100)/100)) + "%" +
		...
		"`\n\n_" + init.Format("15:04:05 02-01") + "_"
